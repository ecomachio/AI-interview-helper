...
const PROMPT = `
Extract a list of technologies from the following job requirements. Return the technologies in raw text format ordered by importance, with each entry consisting of a name and a short description. The description should be a concise 1 to 10-word explanation of the technology's purpose.
The list should contain 10 technologies or less.
Do not include any text other than the list. Ensure the format is exactly as follows:
...
`
const AFTER_PROMPT = `
Important: If multiple technologies are very similar or represent overlapping concepts, consolidate them into the most comprehensive or important technology. Reduce redundancy by selecting the most significant technology that captures the core functionality of similar items.
`
...
export async function POST(req: Request) {
  ...
  const result = await generateText({
    model,
    prompt: `${PROMPT} ${input} ${AFTER_PROMPT}`,
    maxTokens: 1000,
    temperature: 0.2,
    topP: 1,
    frequencyPenalty: 1,
