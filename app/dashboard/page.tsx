...
const Dashboard = () => {
  ...
  const [topics, setTopics] = useState([
    { name: "Data Structures", icon: Database, progress: 75, completed: false },
    { name: "Algorithms", icon: Code, progress: 60, completed: false },
    { name: "System Design", icon: Layout, progress: 40, completed: false },
    { name: "Operating Systems", icon: Server, progress: 30, completed: false },
    { name: "Computer Networks", icon: Network, progress: 25, completed: false },
  ])
  ...
  const [schedule] = useState([
    ...
    { day: "Thursday", topic: "Operating Systems", duration: "2 hours" },
    { day: "Friday", topic: "Computer Networks", duration: "1.5 hours" }
  ])

  const handleTopicCompletion = (index: number) => {
    const newTopics = [...topics]
    newTopics[index].completed = !newTopics[index].completed
    newTopics[index].progress = newTopics[index].completed ? 100 : Math.max(0, newTopics[index].progress - 25)
    setTopics(newTopics)

    const totalProgress = newTopics.reduce((sum, topic) => sum + topic.progress, 0) / newTopics.length
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-4">
                  <Checkbox
                    ...
                    onCheckedChange={() => handleTopicCompletion(index)}
                  />
